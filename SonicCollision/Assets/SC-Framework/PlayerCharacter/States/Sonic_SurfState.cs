...
public class Sonic_SurfState : IState
{
    private readonly Sonic_PlayerStateMachine _ctx;
    private bool _groundDetected;
    private float _skid;
    public Sonic_SurfState(Sonic_PlayerStateMachine _machine)
    {
        _ctx = _machine;
    }
    ...
    private void Movement(float _delta)
    {
        ...
        if (_ctx.InputVector.magnitude > 0.1f && Vector3.Angle(_ctx.HorizontalVelocity.normalized, _ctx.InputVector) > _ctx.Chp.SurfSkidThreshold)
        {
            //_ctx.PlayerDirection = _ctx.InputVector;

            float _turnDeceleration = _ctx.Chp.TurnDeceleration.Evaluate(Vector3.Dot(_ctx.HorizontalVelocity.normalized, _ctx.InputVector)) * _delta * _ctx.Chp.SurfSkidDeccelerationMultiplier;
            _ctx.HorizontalVelocity = Vector3.MoveTowards(_ctx.HorizontalVelocity, Vector3.zero, _turnDeceleration);

            float _acceleration = _ctx.Chp.SurfSkidAcceleration * _delta;
            //_ctx.HorizontalVelocity = Vector3.ClampMagnitude(_ctx.HorizontalVelocity + _ctx.InputVector * _acceleration, _ctx.Chp.BaseSpeed);
            _ctx.HorizontalVelocity += _ctx.InputVector * _acceleration;
        }
        else
        {
            if (_ctx.HorizontalVelocity.magnitude < _ctx.Chp.MinSurfSpeed)
            {
                float _acceleration = _ctx.Chp.SurfAcceleration * _delta;
                //_ctx.HorizontalVelocity = Vector3.ClampMagnitude(_ctx.HorizontalVelocity + _ctx.PlayerDirection * _acceleration, _ctx.Chp.BaseSpeed);
                _ctx.HorizontalVelocity = _ctx.HorizontalVelocity + _ctx.PlayerDirection * _acceleration;
            }
    ...
    private void GroundSwitchConditions()
    {
        if (_ctx.Input.JumpInput.WasPressedThisFrame())
        {
            ...
            _ctx.VerticalVelocity += _ctx.GroundNormal * _ctx.Chp.JumpForce;
            _ctx.Physics_ApplyVelocity();
