...
public class Sonic_GroundState : IState
{
    ...
    private bool GroundCheck()
    {
        ...
        return _groundDetected && Vector3.Angle(_ctx.GroundCast.HitInfo.normal, _ctx.GroundNormal) <= _ctx.Chs.MaxGroundDeviation;
    }

    private bool SlipCheck()
    {
        if (_ctx.HorizontalVelocity.magnitude < _ctx.Chs.MinGroundStickSpeed && Vector3.Angle(-_ctx.Gravity, _ctx.GroundNormal) > FrameworkUtility.ESlipAngle)
        {
            _slipState = _ctx.Chs.SlipTime;
    ...
    private void Movement(float _delta)
    {
        ...
        if (_ctx.InputVector.magnitude > 0.1)
        {
            if (Vector3.Dot(_ctx.HorizontalVelocity.normalized, _ctx.InputVector) < _ctx.Chs.TurnDeviationCap)
            {
                ...
                {
                    _ctx.HorizontalVelocity = Vector3.MoveTowards(_ctx.HorizontalVelocity, Vector3.zero, _ctx.Chs.BreakStrength * _delta);
                    _ctx.Skid = true;
                    return;
                }
                else
            ...
            if (_ctx.HorizontalVelocity.magnitude < _ctx.Chs.BaseSpeed)
            {
                float _acceleration = _ctx.Chs.AccelerationCurve.Evaluate(_ctx.HorizontalVelocity.magnitude) * _delta;
                _ctx.HorizontalVelocity = Vector3.ClampMagnitude(_ctx.HorizontalVelocity + (_acceleration * _ctx.InputVector), _ctx.Chs.BaseSpeed);
            }

            if (!FrameworkUtility.IsApproximate(_ctx.HorizontalVelocity.normalized, _ctx.InputVector, Mathf.Deg2Rad * Mathf.PI))
            {
                float _turnDeceleration = _ctx.Chs.TurnDeceleration.Evaluate(Vector3.Dot(_ctx.HorizontalVelocity.normalized, _ctx.InputVector)) * _delta;
                _ctx.HorizontalVelocity = Vector3.MoveTowards(_ctx.HorizontalVelocity, Vector3.zero, _turnDeceleration);
            }

            float _turnStrength = _ctx.Chs.TurnStrengthCurve.Evaluate(_ctx.HorizontalVelocity.magnitude) * Mathf.PI * _delta;
        ...
        {
            float _deceleration = _ctx.Chs.DecelerationCurve.Evaluate(_ctx.HorizontalVelocity.magnitude) * _delta;
    ...
    private void Slipment(float _delta)
    {
        ...
        _slipState -= _delta;
        _ctx.HorizontalVelocity += Vector3.ProjectOnPlane(_ctx.Gravity, _ctx.GroundNormal) * _ctx.Chs.SlopeFactor;
        _ctx.HorizontalVelocity = Vector3.ClampMagnitude(_ctx.HorizontalVelocity, _ctx.Chs.HardSpeedCap);
    }
    ...
    {
        if (_ctx.HorizontalVelocity.magnitude < _ctx.Chs.MinGroundStickSpeed && Vector3.Angle(-_ctx.Gravity, _ctx.GroundNormal) > FrameworkUtility.SlopeAngle)
        {
            AirSwitchConditions();
        }

        if (_ctx.Input.RollInput.WasPressedThisFrame() && _ctx.HorizontalVelocity.magnitude > _ctx.Chs.MinRollSpeed)
        {
            _ctx.MachineTransition(PlayerStates.Roll);
        }

        if (_ctx.Input.RollInput.IsPressed() && _ctx.Input.JumpInput.WasPressedThisFrame())
        {
